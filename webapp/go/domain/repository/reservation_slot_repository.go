...
type ReservationSlotRepository interface {
	...
	FindAllByRangeForUpdate(ctx context.Context, q Querier, period model.ReservationPeriod) ([]*model.ReservationSlotModel, error)
	...
	// DecrementSlotsByRange は予約区間 period に収まる予約枠の残数を 1 ずつ減らす。
	DecrementSlotsByRange(ctx context.Context, q Querier, period model.ReservationPeriod) error
}
