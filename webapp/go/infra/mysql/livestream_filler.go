...
func fillLivestreams(ctx context.Context, q repository.Querier, livestreamModels []*domain.LivestreamModel, defaultIconHash domain.IconHash) ([]*domain.Livestream, error) {
	ownerModels := make([]*domain.UserModel, len(livestreamModels))
	for i, livestreamModel := range livestreamModels {
		var ownerModel domain.UserModel
	...
	owners, err := fillUsers(ctx, q, ownerModels, defaultIconHash)
	if err != nil {
		return nil, err
	}
	...
	for i, livestreamModel := range livestreamModels {
		...
		tags := make([]domain.TagModel, len(livestreamTagModels))
		...
		livestreams[i] = &domain.Livestream{
