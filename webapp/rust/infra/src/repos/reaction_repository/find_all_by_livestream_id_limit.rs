...
async fn returns_limited_rows() {
...

    {
        ...
        let r1: CreateReaction = CreateReaction { created_at: 100, ..Faker.fake() };
        ins.add_value(&InsertReactionSetup { id: 1, user_id: 1, livestream_id: 1, reaction: &r1 });
        let r2: CreateReaction = CreateReaction { created_at: 300, ..Faker.fake() };
        ins.add_value(&InsertReactionSetup { id: 2, user_id: 1, livestream_id: 1, reaction: &r2 });
        let r3: CreateReaction = CreateReaction { created_at: 200, ..Faker.fake() };
        ins.add_value(&InsertReactionSetup { id: 3, user_id: 1, livestream_id: 1, reaction: &r3 });
        let (sql, binds) = ins.to_sql();
        bind_qbey_values!(sqlx::query(&sql), binds).execute(&mut *tx).await.unwrap();
    }
