...
async fn counts_correctly() {
    ...
    sqlx::query(
        "INSERT INTO livecomments (id, user_id, livestream_id, comment, tip, created_at) VALUES (1, 1, 1, 'a', 0, 100), (2, 1, 1, 'b', 0, 200)",
    )
    .execute(&mut *tx)
    .await
    .unwrap();
    ...
    sqlx::query(
        "INSERT INTO livecomment_reports (id, user_id, livestream_id, livecomment_id, created_at) VALUES (1, 1, 1, 1, 100), (2, 1, 1, 2, 200), (3, 1, 2, 1, 300)",
    )
    .execute(&mut *tx)
    .await
    .unwrap();
