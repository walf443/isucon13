...
async fn counts_correctly() {
    ...
    let comment2: CreateLivestreamComment = Faker.fake();
    {
        ...
        ins.add_value(&[
            ...
            ("comment", comment1.comment.as_str().into()),
            ("tip", comment1.tip.into()),
            ("created_at", comment1.created_at.into()),
        ]);
        ins.add_value(&[
            ("id", 2i64.into()),
            ("user_id", 1i64.into()),
            ("livestream_id", 1i64.into()),
            ("comment", comment2.comment.as_str().into()),
            ("tip", comment2.tip.into()),
            ("created_at", comment2.created_at.into()),
        ]);
        let (sql, binds) = ins.to_sql();
        bind_qbey_values!(sqlx::query(&sql), binds).execute(&mut *tx).await.unwrap();
    }
